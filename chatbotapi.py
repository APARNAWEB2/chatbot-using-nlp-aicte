...
x1=vectorizer.fit_transform(patterns1)
y1=tags1
clf1.fit(x1,y1)

def chatbot(input_text):
    input_text = vectorizer.transform([input_text])
    tag = clf1.predict(input_text)[0]
    for intent in intents:
        if intent['tag'] == tag:
...
def main():
    ...
    st.subheader("Welcome to our chatbot! 😊")
    ...
    if choice == "Home":
        ...
        st.write("It can help u with anything u want, try it on once!😉")
        ...
        if user_input:
            ...
            st.text_area("Chatbot:", value=response, height=120, max_chars=None, key=f"chatbot_response_{counter}☺️")
